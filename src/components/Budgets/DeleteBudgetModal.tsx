...
function DeleteBudgetModal({ closeModal, budget }: DeleteModalProps) {
  const handleDeleteBudget = useFinanceState(state => state.handleDeleteBudget)
  ...
  return (
    <div className="fixed inset-0 bg-black/10 z-30 backdrop-blur-xs h-screen flex items-center justify-center">
      <div className="max-w-140 rounded-xl p-8 flex flex-col gap-5 bg-white w-full">
        ...
        <button onClick={handleDelete} className="rounded-lg bg-red text-white font-bold text-preset4 h-13 w-full">
          Yes, Confirm Deletion
        </button>
        <button onClick={closeModal} className="text-preset4 text-grey500">
          No, Go Back
        </button>
      </div>
    </div>
