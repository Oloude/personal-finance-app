...
function CategoryDropdown() {
  ...
  const handleCatgoryChange = useFinanceState(state => state.handleCatgoryChange)
  ...
  return (
    <div className="flex flex-col items-start gap-3 px-5 py-3 divide-y divide-grey100 absolute top-6 md:top-13 right-0 z-10 bg-white rounded-lg shadow-dropdown">
      ...
      {['All Transactions', ...uniqueCategory].map((currentCategory) => (
        <button onClick={()=> handleCatgoryChange(currentCategory)} className={`text-preset4 text-grey900 pb-3 ${category === currentCategory ? 'font-bold' : ''}`}>{currentCategory}</button>
      ))}
    </div>
  );
}
